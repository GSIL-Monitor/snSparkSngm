...
//    do lazy query and transform
    ...
    val df1 = dfOriginalResMap.filter(col("distance")<=5)
                  .repartition(col("city_cd"))  // repartition big df
...
//    use spark core  window function to get summary
    val dfPayBrandDetail = dfPay5km.union(dfPay1km).repartition(col("city_cd"),col("str_type"),col("str_cd"),col("brand_cd"),col("brand_nm"))
    val rankSpec = Window.partitionBy("city_cd","str_type","str_cd","brand_cd","brand_nm")
                          .orderBy(col("statis_date").asc).rowsBetween(duration+1,0)
    val dfQuantile1 = dfPayBrandDetail.withColumn("pay_amnt",sum("pay_amnt").over(rankSpec))
...
//    do lazy query and transform
    ...
    val dfPayByRes = spark.sql(queryPayByRes).union(spark.sql(queryPayByResComp))
                  .repartition(col("city_cd"),col("dept_cd"),col("res_cd"))
    val dfPayByResBrand = dfPayByRes.groupBy("city_cd","res_cd","dept_cd","brand_cd","brand_nm")
...
//    do join to limit pay amount in 5/3/1 km
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    create temp table
    spark.sql("use sngmsvc")
//    spark.sql("set spark.sql.auto.repartition=true")
    spark.sql(
